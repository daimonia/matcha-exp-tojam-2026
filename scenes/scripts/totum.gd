...
@export var mouse_drag_acceleration = 20.0
@export var drag = 0.9
...
func _physics_process(delta: float) -> void:
	match state:
		State.Dragging:
			var velocity_vector = get_global_mouse_position() - global_position
			apply_force(velocity_vector * mouse_drag_acceleration)
		_:
